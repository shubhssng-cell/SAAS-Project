...
```
**Phase 5A** ([PHASE_5A_REVIEW.md](PHASE_5A_REVIEW.md)) implemented the OBSERVATION -> EVIDENCE half of this table's eventual content as a pure, database-free package, `packages/domain/autopsy` — `buildAutopsyOutput()` produces exactly the kind of structured signal `evidence_used` above is meant to hold (deterministic behavior signals, historical/repeated-evidence counts, and a CANDIDATE — never confirmed — error-category match against this same `ErrorTaxonomy` table). **Phase 5B** ([PHASE_5B_REVIEW.md](PHASE_5B_REVIEW.md)) implemented the HYPOTHESIS + CONFIRMED DIAGNOSIS halves in the same package: `generateHypothesis()` produces a real `hypothesis_text`/`likely_root_cause` (via a 5th `@ipmat/ai` task, docs/AI_ARCHITECTURE.md §10a), and `applyConfirmationResponse()` is the deterministic state machine behind `confirmed`/`student_correction_text`. Inspecting this schema found it needs **no migration at all**: `confirmed: bool | null` + `student_correction_text: text | null` already jointly encode all 4 states a hypothesis can be in (awaiting/confirmed/rejected/corrected — see `toAutopsyPersistenceRecord()` in `packages/domain/autopsy/src/persistence.ts` for the exact mapping, docs/DECISIONS.md D-039). `RepairPlan`'s `follow_up_question_ids` still stays `[]` — `buildRepairPlan()` (Phase 5B) identifies a target (concept/pattern family/taxonomy cell/error category/training mode/priority/prerequisites) from a CONFIRMED hypothesis specifically, but still does not select or generate actual questions. **No `Autopsy` or `RepairPlan` row is ever written by any package in this repository** — `toAutopsyPersistenceRecord()`/`toRepairPlanPersistenceRecord()` are pure mapping functions only; the actual `prisma.autopsy.create()`/`prisma.repairPlan.create()` persistence adapter, the confirm/correct UI, and actual follow-up-question selection are Phase 5C.
...
MasteryState
  ...
  component_detail: jsonb            -- Phase 5B: the rich, never-discarded breakdown these 5 scalars were computed from —
                                      -- counts, speed statistics, accuracy STABILITY (stddev, not just mean), difficulty/
                                      -- novelty/pressure breakdowns, error-recurrence streaks, coverage detail (see
                                      -- @ipmat/mastery's MasteryComponentDetail). Migration 0004_mastery_component_detail;
                                      -- the 5 scalar columns above are UNCHANGED from Phase 1.
  computed_at
```
No `confidence` column exists anywhere in this schema, by design (see [PRODUCT_SPEC.md](PRODUCT_SPEC.md) §5) — and Phase 5B's `component_detail` blob is no exception: it holds counts and statistics, never a confidence/motivation/emotion field (docs/PHASE_5B_REVIEW.md). **Phase 5B** ([PHASE_5B_REVIEW.md](PHASE_5B_REVIEW.md)) implemented the actual, multidimensional `MasteryState` computation as a pure, database-free package, `packages/domain/mastery` — `computeMasteryState()`/`computePatternFamilyMasteryDetail()`/`computeTaxonomyCellMasteryDetail()`. Each of the 5 scalar measures above is independently nullable ("insufficient data," gated on a centralized `MIN_OBSERVATIONS_FOR_COMPONENT` threshold, docs/DECISIONS.md D-041) rather than a fabricated 0; `toMasteryStatePersistenceRecord()` correspondingly writes NOTHING (returns `null`) rather than coercing a misleading default when any of the 5 is still unknown (D-042). No row is ever actually written by any package in this repository — no persistence adapter exists yet (Phase 5C).
