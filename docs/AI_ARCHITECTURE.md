...
Attempt (wrong answer)
  → assemble evidence: chosen_answer vs correct_answer, question DNA (esp. trap_type/pattern),
    time_spent vs expected_time, hints_used, retries, recent Autopsy history on related concepts
  → generateStructured("autopsy-hypothesis", schema=AutopsyHypothesisSchema)
       ...
       - schema forces a `hypothesis_text` field ending in a question back to the student, and an `error_type`
         drawn from a controlled taxonomy (not free text) so downstream repair logic can key off it
  ...
  → on confirm: RepairPlan generated by selecting/generating PatternTaxonomyCell-linked questions
    that target the confirmed error_type for the confirmed concept
```
