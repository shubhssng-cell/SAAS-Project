...
**Update (Phase 3.1.1): every finding in §10 below has been addressed — see the `[Phase 3.1.1: ...]` tag on each item, docs/DECISIONS.md D-027 through D-033, and [PHASE_3_1_1_REVIEW.md](PHASE_3_1_1_REVIEW.md) for the full implementation-level account (exact diffs, tests added, what a fix does and does not cover). Six items are marked `FIXED`; one (stem-embedded answer leakage beyond a verbatim match) is marked `INTENTIONALLY UNRESOLVED`, with the reasoning recorded inline. The rest of this document — §1 through §9, and the narrative text of §10 itself — is left exactly as originally written, as the historical record of what this review found; only the status tags are new.
...
1. **The budget circuit breaker silently no-ops for any model not in the hand-maintained pricing table** (§6, finding 1). This is the single most consequential finding in this review — a safety feature that appears tested and working can become completely inert with a model-name typo or an untracked model, with no error or warning anywhere.
   **[Phase 3.1.1: FIXED.]** `isKnownModel()` (`@ipmat/ai`) is checked before any AI call in `runGenerationPipeline()`; an unpriced model is refused outright with `unverifiable_cost`, never silently treated as free. docs/DECISIONS.md D-027; tests in `generationPipeline.test.ts` (known-model budget correctness, unknown-model refusal, repeated-invocation refusal) and `costEstimation.test.ts` (`isKnownModel`).
2. **The Lens comparison report's "invented relationship" classification has no name normalization** (§5, finding 1) — a real model returning slightly different capitalization or punctuation for a concept name would be reported as having invented a relationship it didn't invent, undermining the credibility of a report whose entire purpose is catching AI overclaiming.
   **[Phase 3.1.1: FIXED.]** `normalizeConceptNameKey()` (`@ipmat/concept-graph`) — exact match after trim/whitespace-collapse/lowercase, never fuzzy — used for all three combination-category lookups in `buildLensComparisonReport()`. docs/DECISIONS.md D-030; tests in `comparisonReport.test.ts` and a dedicated `normalizeConceptNameKey.test.ts`.
3. **`verifyComputation`'s arithmetic allowlist has no bound on computation size/magnitude and an untested interaction between the allowed `,` character and mathjs's actual multi-statement grammar** (§2, findings 1–2) — worth either removing `,` from the allowlist or adding an explicit test proving its behavior is benign, plus considering a length cap and/or timeout around `evaluate()`.
   **[Phase 3.1.1: FIXED.]** `,` removed from the allowlist entirely (its assumed "thousands separator" behavior was never real, per mathjs's actual grammar). New pre-evaluation bounds: 200-character length cap, 15-digit numeric-literal cap, 3-operator exponentiation-complexity cap, and a post-evaluation `1e12` result-magnitude cap. No timeout was added (synchronous `evaluate()` can't easily support one without a worker thread — the magnitude/complexity bounds address the same DoS concern at the input level instead). docs/DECISIONS.md D-028; 9 new tests in `validateCandidate.test.ts`.
4. **The budget safety story depends on an undocumented cross-file coupling**: `AnthropicProvider`'s hardcoded `max_tokens: 4096` is what actually bounds worst-case per-call cost today, not anything in `generationLimits.ts` (§6, finding 3). If either file changes without the other being reconsidered, the real risk profile changes silently.
   **[Phase 3.1.1: FIXED.]** The constant is now named and exported (`ANTHROPIC_MAX_OUTPUT_TOKENS_PER_CALL`), cross-referenced from both files, and the coupling is a computed, tested number (`worstCaseSingleCallCostUsd()`), not just a comment — a regression test would fail if the coupling's assumption ever breaks. docs/DECISIONS.md D-031.
5. **`distractor_quality` is dead code with zero test coverage** (§3, finding 2) — cheap to fix (one new fixture), worth doing before relying on it.
   **[Phase 3.1.1: FIXED.]** Kept (it's a real, distinct quality check — duplicate wrong-answer options are a genuine defect independent of whether the correct answer is fine), and given a dedicated fixture (`duplicateDistractorCandidate`) and test. docs/DECISIONS.md D-033.
6. **The generation system prompt doesn't mention 4 of the 7 fields blueprint compliance now enforces** (§8, finding 1) — not a safety issue, but will cause avoidable rejections; a one-line prompt update would likely reduce them.
   **[Phase 3.1.1: FIXED.]** `buildGenerationSystemPrompt()` now explicitly enumerates all 7 fields; the deterministic validator itself was not touched or relaxed. docs/DECISIONS.md D-032.
7. **Stem-embedded answer leakage has no mitigation anywhere** (§1, finding 1) — this is the most fundamental of the findings and the least straightforward to fix: the independent verifiers need the real stem, and the real stem is exactly where a poorly-generated question could give away its own answer. Worth a deliberate decision (accept the risk for now vs. add a "does the stem itself state the answer" check to the judge's responsibilities) rather than leaving it implicit.
   **[Phase 3.1.1: PARTIALLY FIXED, rest INTENTIONALLY UNRESOLVED.]** A new deterministic check (`validateNoAnswerLeakageInStem()`) catches the concrete, checkable case of a verbatim whole-token leak of `correctAnswer` or `blueprintId` in the stem — this is a real, tested fix for the literal-leak case shown as an example in this finding. What remains unresolved, deliberately: a paraphrased leak (the number stated in words), a differently-formatted leak, or an answer that's algebraically derivable from other stated givens — none of these are deterministically detectable without either a semantic-understanding component this codebase doesn't have, or a keyword blacklist explicitly rejected as the wrong tool (Phase 3.1.1 §3 instructions were explicit on this point). This is documented as a known, accepted limitation in the check's own doc comment, docs/QUESTION_ENGINE.md §5b, and docs/DECISIONS.md D-029 — not silently left implicit as it was before.

All seven items above were fixed (six fully, one partially with the remainder explicitly and deliberately left unresolved) in Phase 3.1.1 — see [PHASE_3_1_1_REVIEW.md](PHASE_3_1_1_REVIEW.md) for the full implementation-level account. Nothing was fixed as part of producing the ORIGINAL version of this document, per that review's own instruction not to modify implementation — these tags were added afterward, once a separate phase (3.1.1) actually did the work.
