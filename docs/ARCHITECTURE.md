...
/packages
  /domain              Framework-agnostic business logic, split by bounded context:
    ...
    (Pressure Training, Phase 5E-6: explicitly DEFERRED, no package -- the current schema has no persisted session/sequence/block identity to build a genuinely distinct provider on; the one safely-available pressure-adjacent signal, testingModes' time_pressured, is already /speed-lab's time_constrained stage. See docs/DECISIONS.md D-059 for the full reasoning and the documented minimum future prerequisite, a general Practice Session/Block abstraction -- now built as /practice-session + /practice-block below, Phase 5F/D-060. Pressure Training ITSELF remains unbuilt.)
    /practice-session     [built, Phase 5F -- general infrastructure only, no Pressure Training/Mock/UI] PracticeSession lifecycle (active/completed/abandoned, D-035 discipline), owned only via enrollmentId -> Enrollment.studentId. ZERO dependencies -- does not depend on /practice-block, /attempt, or anything else; cross-entity checks (e.g. "no active child block") take the fact as a caller-supplied boolean
    /practice-block       [built, Phase 5F -- general infrastructure only, no Pressure Training/Mock/UI] PracticeBlock lifecycle + derived progress/time functions (wall-clock duration, active solving time, inter-attempt gaps -- three independently-derived, non-summing measures). ZERO dependencies, deliberately not depending on /practice-session either -- see docs/DECISIONS.md D-060
