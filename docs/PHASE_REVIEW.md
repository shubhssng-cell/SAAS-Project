...
6. **`correctAnswer` numeric parsing is fragile** (§8.6) — a candidate with a non-numeric or oddly-formatted answer silently skips part of the cross-check rather than failing closed.
7. **The Lens comparison's `supportedByGraph` check is type-blind** (§6.3) — it would count a match against a `related_but_distinct` edge as "supported," which arguably it should not be, since that edge type exists specifically to say "don't combine these."
...
9. **Single demonstration question per phase.** Phase 2 has exactly one hand-authored, fully-worked question. Phase 3 has exactly one AI-attempted blueprint (via fixture). Neither constitutes evidence that the approach scales past a hand-picked favorable example.
...
**92 tests pass. Here is what that does and does not mean.**
...
- **No live AI provider call has ever been made.** `AnthropicProvider` exists as code and has never executed.
