...
import type { CanonicalQuestion, QuestionReader } from "@ipmat/db";

export type { AttemptOwnershipClaim, AttemptQuestionContext, AttemptState, CanonicalQuestion, QuestionReader, RecordableAttemptEventInput };
...
export const PRACTICE_LOOP_ERROR_CODES = ["question_not_published", "question_not_found"] as const;
export type PracticeLoopErrorCode = (typeof PRACTICE_LOOP_ERROR_CODES)[number];

/** Mirrors `@ipmat/attempt`'s `AttemptLifecycleError`/`@ipmat/db`'s `PersistenceError` — a typed, fail-closed error for THIS layer's two genuinely new business rules (no such question; question not published). Every other failure (attempt not found, ownership mismatch, malformed event, missing reference) is left to propagate as the domain/persistence layer's OWN error type, never re-wrapped, so this layer never duplicates error semantics those layers already own. */
