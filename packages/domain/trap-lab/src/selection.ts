...
import { computeLocalExposureCounts, computeSeenTaxonomyCellIds } from "./exposure.js";
import type { TrapLabRequirement } from "./types.js";

function isStructurallyValid(candidate: TrainingCandidateQuestion): boolean {
  return Boolean(candidate?.question?.questionId && candidate.question.conceptName && candidate.question.patternTaxonomyCellId && typeof candidate.expectedTimeSeconds === "number");
}

function qualifies(candidate: TrainingCandidateQuestion, requirement: TrapLabRequirement): boolean {
  if (candidate.validationState !== "published") return false;
  if (candidate.question.trapErrorTaxonomyCode !== requirement.targetErrorTaxonomyCode) return false;
  // Concept filter is applied ONLY when a caller explicitly supplied one -- V1's evaluate() never
  // does, so default Trap Lab selection is cross-concept (docs/DECISIONS.md D-056).
  if (requirement.targetConceptName !== undefined && candidate.question.conceptName !== requirement.targetConceptName) return false;
  return true;
}

/**
 * Deterministic, fully bounded tie-break over the QUALIFYING pool only
 * (docs/DECISIONS.md D-056): (1) prefer a candidate whose taxonomy cell
 * this student has NOT previously attempted against this trap code (a
 * boolean partition, never a score -- "repeat the trap, vary the
 * surface"), (2) least prior exposure for this student, (3) lexicographic
 * questionId as the final deterministic break. No randomization, no
 * weighted/composite score, no delegation to a general ranking engine.
 */
...
/**
 * The selection step, called ONLY after `evaluateTrapLab()` has already
 * decided applicability (never re-decides it -- see
 * `runTrainingSystemProvider()` in `@ipmat/training-systems`).
 */
export function selectTrapLabQuestion(providerId: string, context: TrainingSystemContext, requirement: TrapLabRequirement, explanation: string): TrainingSystemSelectionOutcome {
  ...
  if (qualifying.length === 0) {
    return {
      ...
      requirement,
      explanation: `No published, structurally valid candidate carries trap-taxonomy code "${requirement.targetErrorTaxonomyCode}"${requirement.targetConceptName ? ` for concept "${requirement.targetConceptName}"` : ""}.`,
      diagnostics
    };
  }

  const seenCellIds = computeSeenTaxonomyCellIds(context.studentId, context.attemptRecords, requirement.targetErrorTaxonomyCode);
