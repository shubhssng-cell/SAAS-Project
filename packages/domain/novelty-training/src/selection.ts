...
import { computeLocalExposureCounts, computeSeenTaxonomyCellsAtLevel } from "./exposure.js";
...
function isStructurallyValid(candidate: TrainingCandidateQuestion): boolean {
  return Boolean(candidate?.question?.questionId && candidate.question.conceptName && candidate.question.noveltyLevel && candidate.question.patternTaxonomyCellId && typeof candidate.expectedTimeSeconds === "number");
}

function qualifies(candidate: TrainingCandidateQuestion, requirement: NoveltyTrainingRequirement): boolean {
  ...
  if (candidate.question.noveltyLevel !== requirement.targetNoveltyLevel) return false;
  return true;
}

/**
 * Deterministic, fully bounded tie-break over the QUALIFYING pool only
 * (docs/DECISIONS.md D-058): (1) prefer a candidate whose taxonomy cell
 * this student has NOT previously attempted at this target novelty level
 * (surface variation), (2) least prior exposure for this student, (3)
 * lexicographic questionId as the final deterministic break. No
 * weighting, no composite score, no global ranking.
 */
...
/**
 * The selection step, called ONLY after `evaluateNoveltyTraining()` has
 ...
 */
export function selectNoveltyTrainingQuestion(providerId: string, context: TrainingSystemContext, requirement: NoveltyTrainingRequirement, explanation: string): TrainingSystemSelectionOutcome {
  ...
  if (qualifying.length === 0) {
    return {
      ...
      requirement,
      explanation: `No published, structurally valid candidate matches concept "${requirement.targetConceptName}" at novelty level "${requirement.targetNoveltyLevel}".`,
      diagnostics
    };
  }

  const seenCellIds = computeSeenTaxonomyCellsAtLevel(context.studentId, context.attemptRecords, requirement.targetConceptName, requirement.targetNoveltyLevel);
